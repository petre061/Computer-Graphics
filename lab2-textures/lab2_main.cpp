...
GLuint positionBuffer, colorBuffer, indexBuffer, vertexArrayObject, texture, textCordBuffer, vertexArrayObject2, texture2;
...
void initGL()
{
	...
	//Set up the attrib pointer.
	//first param = texcoords buff is in inded 2
	//second param = texcoords element consists of 2 floats (u and v)
	glVertexAttribPointer(2, 2, GL_FLOAT, false/*normalized*/, 0/*stride*/, 0/*offset*/);

	//Enable the vertex attrib array.
	//textcoords are in location 2
	...
	const float explosionPos[] = {
		// X		Y			Z
		-25.0f,	-10.0f,	-100.0f,  // v0
		-25.0f,	40.0f,	-100.0f, // v1
		25.0f,  -10.0f,	-100.0f, // v2
		25.0f,  40.0f,  -100.0f   // v3
	};
	...
	glBindBuffer(GL_ARRAY_BUFFER, positionBuffer);
	// Send the vetex position data to the current buffer
	glBufferData(GL_ARRAY_BUFFER, sizeof(explosionPos), explosionPos, GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, false /*normalized*/, 0 /*stride*/, 0 /*offset*/);
	// Enable the attribute
	glEnableVertexAttribArray(0);

	// Indicses for the explosion two triangles
	...
	//generate OpenGL texture and initialize it with the texture data
	glGenTextures(1, &texture);

	//bind texture, allocate storage and upload data
	glBindTexture(GL_TEXTURE_2D, texture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	free(image);

	//define where to sample when a coordinate is out of range
	//start by specifying OpenGL to clamp the texture coordinate within range
	//range is between (0,0) = bottum left and (1,1) = upper right
	...
	//clamps to edge when out of range, created a stretched out image
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

	//setup texture filtering
	//texture filtering always needs to be set, otherwise can cause problems 
	...
	//enables nice anisotropic filtering 
